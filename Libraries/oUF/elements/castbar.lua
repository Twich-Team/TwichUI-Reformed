...
local function CastStart(self, event, unit)
	...
	local name, text, texture, startTime, endTime, isTradeSkill, _, notInterruptible, spellID, castID = UnitCastingInfo(
	unit)
	if (name) then
	...
	if (safeZone and unit == 'player') then
		...
		if (element.channeling) then
			safeZone:SetPoint(element:GetReverseFill() and (isHoriz and 'RIGHT' or 'TOP') or
			(isHoriz and 'LEFT' or 'BOTTOM'))
		else
			safeZone:SetPoint(element:GetReverseFill() and (isHoriz and 'LEFT' or 'BOTTOM') or
			(isHoriz and 'RIGHT' or 'TOP'))
		end
				...
				if (self.delay ~= 0) then
					...
					* duration - a [Duration](https://warcraft.wiki.gg/wiki/ScriptObject_DurationObject) object for the Castbar
					...
					if (self.CustomDelayText) then
						...
						self.Time:SetFormattedText('%.1f|cffff0000%s%.2f|r', duration, self.channeling and '-' or '+',
							self.delay)
					end
				else
