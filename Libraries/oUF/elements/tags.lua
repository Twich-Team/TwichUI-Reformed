...
local tagStrings = {
	...
	['name']                = [[function(u, r, length)
		local name = UnitName(r or u)
		local maxLength = tonumber(length)

		if(name and maxLength and maxLength > 0) then
			if((issecretvalue and issecretvalue(name)) or (hasanysecretvalues and hasanysecretvalues(name))) then
				return name
			end
	...
	if (not func) then
		...
		for bracket in tagstr:gmatch(_PATTERN) do
			...
			if (not tagFunc) then
				...
				if (tag) then
					local dynamicTag = tag
					---@cast dynamicTag fun(...): any
					if (prefixEnd ~= 1 or suffixStart - suffixEnd ~= 1) then
						...
						tagFunc = function(unit, realUnit)
							local str
							if (customArgs) then
								str = dynamicTag(unit, realUnit, string.split(',', customArgs))
							else
								str = dynamicTag(unit, realUnit)
							end
