...
local MINOR_VERSION = 9999 -- TwichUI patched: bumped to ensure this copy wins LibStub over ElvUI_Libraries
...
-- WoW Midnight removed SecureHandlerBaseTemplate:WrapScript / :UnwrapScript as instance methods.
-- Fall back to the global SecureHandlerWrapScript / SecureHandlerUnwrapScript if needed.
local function LAB_WrapScript(handler, target, script, preBody, postBody)
	if handler.WrapScript then
		handler:WrapScript(target, script, preBody, postBody)
	elseif _G.SecureHandlerWrapScript then
		_G.SecureHandlerWrapScript(target, script, handler, preBody, postBody)
	end
end

local function LAB_UnwrapScript(handler, target, script)
	if handler.UnwrapScript then
		return handler:UnwrapScript(target, script)
	elseif _G.SecureHandlerUnwrapScript then
		return _G.SecureHandlerUnwrapScript(target, script, handler)
	end
end

local function LAB_SetFrameRef(handler, label, refFrame)
	if handler.SetFrameRef then
		handler:SetFrameRef(label, refFrame)
	elseif _G.SecureHandlerSetFrameRef then
		_G.SecureHandlerSetFrameRef(handler, label, refFrame)
	end
end

local function LAB_Execute(handler, body)
	if handler.Execute then
		handler:Execute(body)
	elseif _G.SecureHandlerExecute then
		_G.SecureHandlerExecute(handler, body)
	end
end
...
local C_TransmogOutfitInfo_IsEquippedGearOutfitLocked = C_TransmogOutfitInfo and
...
local ButtonRegistry, ActiveButtons, ActionButtons, NonActionButtons = lib.buttonRegistry, lib.activeButtons,
	lib.actionButtons, lib.nonActionButtons
...
function SetupSecureSnippets(button)
	...
	LAB_WrapScript(button.header, button, "OnDragStart", [[
	...
	LAB_WrapScript(button.header, button, "OnReceiveDrag", [[
	...
	if UseCustomFlyout then
		LAB_SetFrameRef(button.header, "flyoutHandler", GetFlyoutHandler())
	end
end

function WrapOnClick(button, unwrapheader)
	-- unwrap OnClick until we got our old script out
	if unwrapheader then
		local wrapheader
		repeat
			wrapheader = LAB_UnwrapScript(unwrapheader, button, "OnClick")
	...
	LAB_WrapScript(button.header, button, "OnClick", [[
...
local function UpdateAbilityInfo(self)
	...
	if isTypeAction then
		...
		local actionSpell, actionMacro, actionFlyout = actionType == 'spell', actionType == 'macro',
			actionType == 'flyout'
		local macroSpell = actionMacro and
		((subType == 'spell' and actionID) or (subType ~= 'spell' and GetMacroSpell(actionID))) or nil
...
function Generic:UpdateState(state)
	...
	if self.header then
		LAB_SetFrameRef(self.header, "updateButton", self)
		LAB_Execute(self.header, [[
	...
	local function SyncFlyoutInfoToHandler()
		...
		for flyoutID, info in pairs(lib.FlyoutInfo) do
			if info.isKnown then
				local numSlots = 0
				data = data ..
				("local info = newtable();LAB_FlyoutInfo[%d] = info;info.slots = newtable();\n"):format(flyoutID)
				for slotID, slotInfo in ipairs(info.slots) do
					data = data ..
					("local info = newtable();LAB_FlyoutInfo[%d].slots[%d] = info;info.spellID = %d;info.overrideSpellID = %d;info.isKnown = %s;info.spellName = %s;\n")
					:format(flyoutID, slotID, slotInfo.spellID, slotInfo.overrideSpellID,
						slotInfo.isKnown and "true" or "nil",
						slotInfo.spellName and format('"%s"', slotInfo.spellName) or nil)
					numSlots = numSlots + 1
				end
		...
		LAB_Execute(GetFlyoutHandler(), data)
		...
		if maxNumSlots > #lib.FlyoutButtons then
			for i = #lib.FlyoutButtons + 1, maxNumSlots do
				...
				LAB_SetFrameRef(lib.flyoutHandler, "flyoutButton" .. i, button)
		...
		for flyoutID = 1, 300 do
			...
			if success then
				...
				for slotID = 1, numSlots do
					...
					lib.FlyoutInfo[flyoutID].slots[slotID] = { spellID = spellID, spellName = spellName, overrideSpellID =
					overrideSpellID, isKnown = isKnownSlot }
				end
			end
		end
...
local function UpdateTextElement(button, element, config, defaultFont, fromRange)
	...
	element:SetPoint(config.position.anchor, element:GetParent(), config.position.relAnchor or config.position.anchor,
		config.position.offsetX or 0, config.position.offsetY or 0)
...
function UpdateRange(button, force, inRange, checksRange) -- Sezz: moved from OnUpdate
	...
	button.outOfRange = ((inRange == nil or checksRange == nil) and button:IsInRange() == false) or
...
local function CreateChargeCooldownFrame(parent)
	...
	local cooldown = CreateFrame("Cooldown", "LAB10ChargeCooldown" .. lib.NumChargeCooldowns, parent,
	...
	function UpdateCooldown(self)
		...
		if ActionButton_ApplyCooldown then
			ActionButton_ApplyCooldown(self.cooldown, cooldownInfo, self.chargeCooldown, chargeInfo,
				self.lossOfControlCooldown, lossOfControlInfo)
			...
			local start, duration, enable, modRate = cooldownInfo.startTime, cooldownInfo.duration,
				cooldownInfo.isEnabled, cooldownInfo.modRate
			local charges, maxCharges, chargeStart, chargeDuration, chargeModRate = chargeInfo.currentCharges,
				chargeInfo.maxCharges, chargeInfo.cooldownStartTime, chargeInfo.cooldownDuration,
...
function UpdateFlash(self)
	...
	if C_TransmogOutfitInfo_IsLockedOutfit and self.AutoCastOverlay then
		if self._state_type == 'action' then
			...
			local isLockedEquippedGear = IsEquippedGearOutfitAction(self._state_action) and
			C_TransmogOutfitInfo_IsEquippedGearOutfitLocked()
...
Generic.GetCharges                = function(self)
	...
	if charge then
		return charge.currentCharges, charge.maxCharges, charge.cooldownStartTime, charge.cooldownDuration,
			charge.chargeModRate
	end
end
...
if WoWClassic then
	...
	if LibClassicSpellActionCount then
		...
		Action.IsConsumableOrStackable = function(self)
			return IsConsumableAction(self._state_action) or IsStackableAction(self._state_action) or
...
Spell.IsUnitInRange                     = function(self, unit) return C_Spell.IsSpellInRange(self._state_action, unit) or
	nil end
...
if C_Spell and C_Spell.GetSpellLossOfControlCooldownDuration then
	Spell.GetLoCCooldownDuration = function(self) return C_Spell.GetSpellLossOfControlCooldownDuration(self
		._state_action) end
end
...
Item.GetCooldownInfo           = function(self)
	local start, duration, enable = C_Container_GetItemCooldown(getItemId(self._state_action))
	return { startTime = start, duration = duration, isEnabled = enable, modRate = 1.0, isActive = (enable and start and start > 0 and duration and duration > 0) }
end
