...
function DataTextModule:ShowStandaloneMenu(panel, menuList)
    ...
    if ui.CreateSecureMenu then
        ...
        if self.standaloneMenu then
            ...
            self.standaloneMenu.styleOverride = GetResolvedPanelStyle(panel and panel.ownerPanel and panel.ownerPanel.panelID)
...
function DataTextModule:ApplyStandalonePanelStyle(frame, panelDefinition)
    ...
    for slotIndex = 1, SLOT_COUNT do
        ...
        if slot and slot.text then
            ...
            slot.hoverGlow:SetColorTexture(style.accentColor[1], style.accentColor[2], style.accentColor[3], 0.09)
            slot.hoverBar:SetColorTexture(style.accentColor[1], style.accentColor[2], style.accentColor[3], 0.92)
        end
...
function DataTextModule:ConfigureStandaloneSlot(slot, panelDefinition, slotIndex, segments)
    ...
    local definition = definitionName and definitionName ~= "NONE" and self.DatatextRegistry and self.DatatextRegistry[definitionName] or nil
    ...
    for slotIndex = 1, SLOT_COUNT do
        ...
        if divider then
            if hasAssignments and slotIndex < segments then
                local dividerOffset = FRAME_INSET + (slotIndex * slotWidth) + ((slotIndex - 1) * SLOT_GAP) + (SLOT_GAP * 0.5)
...
function DataTextModule:ApplyStandalonePanelPosition(frame, panelDefinition)
    ...
    frame:SetPoint(panelDefinition.point or "BOTTOM", UIParent,
        panelDefinition.relativePoint or panelDefinition.point or "BOTTOM", panelDefinition.x or 0, panelDefinition.y or 0)
end
