...
function UnitFrames:GetPalette(scopeOrUnitKey, unit, mockClass)
    ...
    -- "Theme" health mode always maps to the theme's accent color. The successColor
    -- (green) is an enabled-state indicator and should never be used as a health color.
    local themeHealthColor = GetThemeColor("accentColor", { 0.96, 0.76, 0.24, 1 })
...
do
    ...
    function UnitFrames:UpdateCastbarSpellStylePreview(frame, template, assignedSpells)
        ...
        castbar:SetStatusBarTexture((settings.texture and settings.texture ~= "" and GetLSMTexture(settings.texture)) or
            GetThemeTexture())
        castbar:SetStatusBarColor(appearance.fillColor[1] or 1, appearance.fillColor[2] or 1,
            appearance.fillColor[3] or 1,
            appearance.fillColor[4] or 1)
        castbar:SetBackdropColor(appearance.backgroundColor[1], appearance.backgroundColor[2],
...
function UnitFrames:OnEnable()
    ...
    -- Test mode must never persist across sessions — always reset it on login.
    -- If the player logged out while the config UI had test mode enabled, the
    -- frames would be stuck showing preview content in the live game.
    do
        local db = self:GetDB()
        db.testMode = false
